...
Post one comment per run, with `mcp__github__add_issue_comment` so that the
author is the maintainer's account and not `claude[bot]`. The last line of the
body you pass is the marker — write no footer and no signature yourself.

### Then strip the footer that gets added for you

The platform appends a `Generated by Claude Code` footer to the comment after
you post it. **That footer must not remain.** It is not yours to write and not
yours to keep, so remove it:

1. Read the comment back, using the id the post returned:
   `gh api repos/$OWNER_REPO/issues/comments/<id> --jq .body`
2. If it ends in a `---` line followed by `_Generated by [Claude Code](…)_`,
   cut exactly that trailer off and write the body back:
   `printf '%s' "$CLEAN" | gh api --method PATCH \
      repos/$OWNER_REPO/issues/comments/<id> -F body=@-`
