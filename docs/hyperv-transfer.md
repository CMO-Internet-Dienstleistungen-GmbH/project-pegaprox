...
conversion writes only the blocks the source actually held, not what the source claimed. A
transfer sized by the virtual figure would have moved 64 GiB in both columns.

Whether the *target* then stays small is a property of the target storage, not of this
transfer — see *On a real Proxmox node* below, where a thick-provisioned volume reserves
its full size before the first byte arrives.
...
quietly and it does not leave a target that would mount. **The wording is not a promise:**
on a block target the same situation reports an I/O error rather than "No space left on
device", measured below. What PegaProx does with that
...
## On a real Proxmox node

Everything above converts into a file inside a container. This section is the same
conversion into a volume Proxmox allocated, on a **Proxmox VE 9.2.11** node with
**qemu-img 11.0.3**, **ZFS 2.4.3** and kernel 6.17.9, reading over a real SMB mount made
with the product's own options. The source is a 64 GiB sparse VHDX holding 1 GiB.

The commands are the product's: `pvesm alloc` for the volume, `pvesm path` for its device,
the same `qemu-img convert`, and `pvesm free` afterwards. The content is read back off the
allocated volume and compared against the fixture checksum.

| | Directory storage | ZFS block storage | ZFS storage capped at 512 MiB |
|---|---|---|---|
| What `pvesm path` returns | a raw file | `/dev/zvol/…`, a block device | `/dev/zvol/…`, a block device |
| Allocated before the conversion | 1 KiB of 64 GiB apparent | 70.8 GiB reserved | thin |
| `qemu-img` exit code | 0 | 0 | 1 |
| Peak resident memory | 37 472 KiB | 37 388 KiB | 37 464 KiB |
| Duration | 11 s | 35 s | 1 s |
| Allocated afterwards | 1 049 289 KiB | 70.8 GiB | freed |
| Content | matches the fixture checksum | matches the fixture checksum | — |
| What it said on failure | — | — | `error while writing at byte 490733568: Input/output error` |

Three things this measured that the container could not.

**Whether the target stays sparse is the storage's decision, not ours.** The directory
storage kept 64 GiB apparent against 1 GiB allocated. The ZFS storage reserved **70.8 GiB
before a single byte was written**, because it was not configured thin, and still held that
afterwards. Plan capacity by the virtual size unless the target storage is explicitly thin
provisioned.

**Running out of room on a block device does not say "No space left on device".** It says
`Input/output error`, because the volume is a block device and the shortage is the pool's,
not the file's. Any message shown to an operator therefore carries qemu-img's own text
rather than a rephrasing, and the partial volume is freed either way.

**Memory does not change with the target type.** Around 37 MiB in all three cases, the same
constant the container measured at a different absolute value on different hardware.

A failed conversion left nothing behind in any of the three runs: `pvesm free` removed the
volume, the share was unmounted and the credentials file deleted.
...
- **Nothing here ran on Windows.** The source is synthetic in every figure above, including
  the Proxmox ones: the share is a stand-in, not a Hyper-V host. The measurements this epic
  still owes a real Hyper-V system are listed in `docs/hyperv-console.md` and
  `docs/hyperv-verification.md`.
- **A share pinned to SMB 3.0 can be refused by a modern SMB server.** The mount options
  name `vers=3.0`, which every supported Windows Server speaks. A Samba 4.22 share
  configured with `server min protocol = SMB3` refuses it, because that alias means 3.1.1
  in current Samba; the mount then fails with `Dialect not supported by server`. This is a
  property of a stand-in share rather than of a Hyper-V host, and it is written down here
  so that a failing testbed is not mistaken for a failing product.
