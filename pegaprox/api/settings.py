...
def get_audit_log_api():
    ...
    if fmt == 'csv':
        ...
        if entries:
            extra = sorted({k for e in entries for k in e.keys()} - set(cols) - {'signature'})
            cols += extra
...
# NS 2026-04-24 — server-side throttle for node update checks.
...
def check_cluster_updates(cluster_id):
    ...
    # NS: serve cached result if < 24h old and caller didn't ask for force
    force = False
    try:
    ...
    def run_rolling_update():
        try:
            ...
            try:
                ...
                _log(f"Cluster state: {len(online_nodes)}/{total_nodes} nodes online · quorum { 'HELD' if quorum_ok else 'AT RISK' }")
            except Exception as e:
                _log(f"Cluster state check skipped: {e}")
            ...
            try:
                ...
                if ceph is None:
                    ...
                    _log(f"Ceph: {badge} {status} · {ceph.get('osd_up', 0)}/{ceph.get('osd_in', 0)} OSDs up"
