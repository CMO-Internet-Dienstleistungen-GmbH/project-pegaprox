...
- The chain ran inside the service's own start. cmd.exe never reports SERVICE_RUNNING,
  ...
  installers run.
- `start "" /wait` did not keep the two installers apart on the guest: both logs start
  two to three seconds after each other and overlap, and which of the two fails varies.
  The script therefore does not rely on a process exiting. Windows Installer holds the
...
Wait-LauncherGone
$ok = $true
# The drivers first: the agent talks over the VirtIO serial port they install.
if (-not (Install-Msi 'virtio-win-gt-x64.msi' 'msi.log' @('ADDLOCAL=ALL'))) { $ok = $false }
