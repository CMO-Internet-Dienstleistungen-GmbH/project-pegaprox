...
def _hyperv_manager(api, *, vms=None, detail=None, iso_paths=('C:\\iso',)):
    ...
    fake.get_vm_config.return_value = detail if detail is not None else _vm_detail()
...
@pytest.mark.parametrize('rule', [
    ...
    '/api/hyperv/<cluster_id>/vms/<int:vmid>/console',
...
def test_no_hyperv_route_can_change_the_source_beyond_preparing_it(api):
    ...
    assert writing == {
        ...
        # A console ticket is a POST because it mints something, and what it mints is a
        # token for this browser. Nothing on the Hyper-V host changes; opening a console
        # is a read of a screen.
        ('/api/hyperv/<cluster_id>/vms/<int:vmid>/console', 'POST'),
