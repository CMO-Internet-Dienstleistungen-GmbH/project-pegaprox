...
def _run_script(tmp_path, exit_codes=None, wait_for_installer=True, fail_always=False,
                staged=('virtio-win-gt-x64.msi', 'qemu-ga-x86_64.msi')):
...
@needs_pwsh
def test_an_iso_with_only_the_x86_driver_installer_gets_that_one_installed(tmp_path):
    """The staging copies whichever driver MSI the ISO carries under its own name. The old
    service command named virtio-win-gt-x64.msi whatever had been staged."""
    done, events, staging = _run_script(
        tmp_path, staged=('virtio-win-gt-x86.msi', 'qemu-ga-x86_64.msi'))

    assert done.returncode == 0, done.stdout + done.stderr
    assert 'invoked virtio-win-gt-x86.msi' in events, events
    assert 'end virtio-win-gt-x86.msi' in events, events
    assert not staging.exists()
