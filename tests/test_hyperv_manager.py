...
class FakeClient:
    ...
    def run_action(self, script, description):
...
class TestParameterBinding:
    ...
    def test_a_vm_id_is_bound_as_a_literal(self):
        manager = _manager({'$vm = Get-VM -Id': fixture('vm_detail_gen2')})
        manager.get_vm(GUID_GEN2)
        assert f"$VmId = '{GUID_GEN2}'" in manager._client.read_scripts[0]

    def test_a_quote_in_a_value_cannot_close_the_string(self):
        # A VM name or ISO path may legally contain a quote.
        assert hyperv._as_powershell_literal("x'; Remove-VM -Name *") == "'x''; Remove-VM -Name *'"

    def test_a_bound_script_still_parses_as_powershell(self):
        # The binding inserts lines after param(); the result has to remain a valid script.
        from pegaprox.core import hyperv_scripts as scripts
        bound = hyperv._with_parameters(scripts.VM_DETAIL, VmId=GUID_GEN2)
        assert bound.splitlines()[0].startswith('param(')
        assert f"$VmId = '{GUID_GEN2}'" in bound

    @pytest.mark.parametrize('value, expected', [
        (None, '$null'), (True, '$true'), (False, '$false'), (42, '42'),
        (['a', 'b'], "@('a','b')"),
    ])
    def test_each_python_type_becomes_the_right_powershell_literal(self, value, expected):
        assert hyperv._as_powershell_literal(value) == expected
